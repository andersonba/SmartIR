...
class SmartIRClimate(ClimateEntity, RestoreEntity):
    ...
    async def async_added_to_hass(self):
        ...
        if last_state is not None:
            ...
            self._current_preset_mode = last_state.attributes.get('preset_mode')
    ...
    def preset_modes(self):
        """Return the preset modes currently supported for this device."""
    ...
    async def send_command(self):
        async with self._temp_lock:
            try:
                ...
                cmd_payload = self._commands[operation_mode][fan_mode]
                if self._support_swing is True:
                    cmd_payload = cmd_payload[swing_mode]
                if self._support_preset is True:
                    cmd_payload = cmd_payload[preset_mode]
                cmd_payload = cmd_payload[target_temperature]

                await self._controller.send(cmd_payload)
